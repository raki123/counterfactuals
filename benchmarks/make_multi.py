...
with open(f"multi_instances/tree_{n}_{k}.lp", 'w') as out_file:
...
for i in range(10):
    ...
    negative = []
    positive = []
    while len(available) > 0 and len(evidence) < 10:
    ...
    path = 'multi_queries.csv'
    if not os.path.exists(path):
        with open(f"multi_queries.csv", 'a') as csvfile:
