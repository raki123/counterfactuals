...
while len(sys.argv) > 1:
    if sys.argv[1].startswith("-"):
        ...
        del sys.argv[1]
with open(f'{base_path}/multi_queries.csv', 'r') as query_file:
    # reading the CSV file
    csvFile = csv.reader(query_file)

    # displaying the contents of the CSV file
    lines = list(csvFile)[1:]
    lines = lines[10*line_nr:10*(line_nr + 1)]
    file = f"{base_path}/multi_instances/{lines[0][0]}"
...
for i,line in enumerate(lines):
    ...
    start = time.time()
    try:
        result = program.multi_query(intervention, evidence, [query], strategy=strategy)
    except:
        logger.info(f"  Found contradictory evidence.")
        end = time.time()
        logger.info(f"  Execution time {i}: {end - start}")
        continue
    end = time.time()
    logger.info(f"  Execution time {i}: {end - start}")
    # print the results
